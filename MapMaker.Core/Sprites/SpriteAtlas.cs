...
public class SpriteAtlas
{
    ...
    public ObservableCollection<SpriteAnimation> Animations { get; set; } = new();

    public Dictionary<string, byte[]> SheetBytes { get; set; } = new();


    public SpriteEntry? GetById(uint id) =>
        Sprites.FirstOrDefault(s => s.Id == id);

    public SpriteAnimation? GetAnimationById(uint id) =>
        Animations.FirstOrDefault(a => a.Id == id);
